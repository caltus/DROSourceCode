	...
	: CDialog(CTestGameDlg::IDD, pParent)
{
	...
	m_pfn_AhnHSUserUtil_SetFolderPermission = NULL;
...
void CTestGameDlg::OnBtnSetpath() 
{
	...
	if ( m_bUseDll == TRUE && m_pfn_AhnHSUserUtil_SetFolderPermission != NULL )
	{
		dwRet = m_pfn_AhnHSUserUtil_SetFolderPermission(szbuf);
	}
	else
	{	
...
void CTestGameDlg::OnCheckUseDll()
{
	...
	m_hUserUtil = LoadLibraryA ( "HsUserUtilDll.dll" );
	if ( m_hUserUtil == NULL )
	{
		AfxMessageBox( "HSUserUtilDll.dll Load Fail. %d", GetLastError() );
		return;
	}
	...
	m_pfn_AhnHSUserUtil_SetFolderPermission = (PFN_AhnHSUserUtil_SetFolderPermission)GetProcAddress ( m_hUserUtil, (const char*)MAKELONG ( 2, 0 ));
	if ( NULL ==  m_pfn_AhnHSUserUtil_SetFolderPermission )
	{
		AfxMessageBox( "m_pfn_AhnHSUserUtil_SetFolderPermission GetProcAddress Fail. %d", GetLastError() );
		m_pfn_AhnHSUserUtil_SetFolderPermission = NULL;
	}
	...
	m_bUseDll = TRUE;
	AfxMessageBox ( "HSUserUtil.dll Load Complete!" );
