                     ...
                     int       nCmdShow)
{
	...
	*pEnd = _T('\0');
	...
	//  - DESCRIPTION:
	//    구서버연동 적용시,
	//    보호하고자 하는 게임 클라이언트내의 함수 포인터를 인자로 받아 해당 함수들의
	//    CRC를 HackShield.crc 형태의 파일로 저장합니다. 메모리 무결성을 보장하기 위해 
	//    서버에서는 이 함수를 통해 생성된 HackShield.crc 파일을 관리해야 합니다. 
	//    함수의 인자로 함수 포인터를 넘길 경우 최대 32개까지의 함수를 보호할 수 있으며 
	//    첫 번째 인자로 넘어가는 함수의 개수와 뒤에 오는 함수 포인터의 개수가 정확하게 일치해야 합니다.
	
	//    ※주의
	//      이 함수는 EXE에 존재하는 함수여야 하며 동일한 이름을 가진 함수들(오버로딩)에 대해서는 지원하지 않습니다.

	//  - SYNTAX:
	//	  int __stdcall _AhnHS_SaveFuncAddress (IN unsigned int unNumberOfFunc, ...);
	...
	//     ERROR_ANTICPCNT_SAVEFUNCADDRESS_INVALIDPARAM
	//	   입력 값이 올바르지 않습니다. (Value = 0x10020)
	//	   ERROR_ANTICPCNT_SAVEFUNCADDRESS_INITCRYPT_FAIL
	//     암호화 초기화에 실패했습니다. (Value = 0x10023)
	//	   ERROR_ANTICPCNT_SAVEFUNCADDRESS_DECRYPTMESSAGE_FAIL
	//	   메시지 복호화에 실패했습니다. (Value = 0x10024)
	//     ERROR_ANTICPCNT_SAVEFUNCADDRESS_ENCRYPTMESSAGE_FAIL
	//     메시지 암호화에 실패했습니다. (Value = 0x10029)

	int nResult = _AhnHS_SaveFuncAddress( 10,
										  AhnHS_Callback,
										  NetworkThreadProcClient,
										  _AhnHS_Initialize,
										  _AhnHS_StartService,
										  _AhnHS_StopService,
										  _AhnHS_Uninitialize,
	...
	TRACE("[%x] = _AhnHS_SaveFuncAddress()", nResult);
...
void GetEnvInfoFromIni(void)
{
	// 확장서버연동 사용 유무 정보 읽음. (TRUE = 확장서버연동, FALSE = 구 서버연동)
	g_bUseAntiCpX = ::GetPrivateProfileInt(_T("ServerInfo"), _T("UseAntiCpX"), 0, g_szIniPath);
	// 서버 포트
...
int __stdcall AhnHS_Callback(long lCode, long lParamSize, void* pParam)
{
	switch(lCode)
	{
		...
		{
			MessageBox(NULL, _T("프로그램에 대하여 창모드 해킹툴이 감지되었습니다."), szTitle, MB_OK);
			PostThreadMessage(g_dwMainThreadID, WM_QUIT, 0, 0);
			break;
		}
		//Speed 관련
...
UINT WINAPI NetworkThreadProcClient ( LPVOID lParam )
{
	...
	while ( TRUE )
	{
		...
		else
		{
			switch ( header.opCode )
			{
				case OP_REQGUID :
				{
					...
					// -------------------------------------------------------------------------
					// - DESCRIPTION: 
					//	  클라이언트에서 사용하며 서버에서 전달한 암호화된 버전 요구 메시지를 
					//    복호화하고 현재 클라이언트 파일의 버전을 암호화하여 응답 메시지를 만듭니다.
					//
					// - SYNTAX:
					//	int __stdcall _AhnHS_MakeGuidAckMsg (	IN unsigned char *pbyGuidReqMsg,
					//											OUT unsigned char *pbyGuidAckMsg);
					//
					// - PARAMETERS:
					//		.pbyGuidReqMsg	unsigned char *			:서버에서 보낸 암호화된 버전 요구 메시지로 
					//												 버퍼의 크기는 HShield.h에 정의된 SIZEOF_GUIDREQMSG
					//												 이어야 합니다.
					//      .pbyGuidAckMsg	unsigned char *			:서버로 보낼 암호화된 버전 응답 메시지로 버퍼의 크기는 
					//												  HShield.h에 정의된 SIZEOF_GUIDACKMSG이어야 합니다.
					// - RETURN VALUE:
					//     . ERROR_SUCCESS      : 성공
					//     . 기타				: 기타 리턴값에 대해서는 핵쉴드 프로그래밍 가이드 참고바람.
					...
					TRACE("[%x] = _AhnHS_MakeGuidAckMsg()", dwRet);

					// Header를 작성한다.
					header.opCode = OP_ACKGUID;
					header.length = htonl ( SIZEOF_GUIDACKMSG );
				...
				case OP_REQCRC :
				{
					...
					// -------------------------------------------------------------------------
					// - DESCRIPTION: 
					//	  클라이언트에서 사용하며 서버에서 전달한 암호화된 CRC 요구 메시지를 복호화하고 
					//	  게임 파일의 각종 정보를 구해 암호화된 CRC 응답 메시지를 생성합니다.
					//
					// - SYNTAX:
					//		int __stdcall _AhnHS_MakeAckMsg (unsigned char *pbyRequest, unsigned long ulRequestLength, 
					//									PAHNHS_TRANS_BUFFER pResponseBuffer);
					// - PARAMETERS:
					//	    .pbyReqMsg	unsigned char *			: 서버에서 보낸 암호화된 CRC 요구 메시지로 버퍼의 크기는
					//											  HShield.h에 정의된 SIZEOF_REQMSG이어야 합니다.
					//		.pbyAckMsg	unsigned char *			: 서버로 보낼 암호화된 CRC 응답 메시지로 버퍼의 크기는 
					//											  HShield.h에 정의된 SIZEOF_ACKMSG 이어야 합니다.
					//
					// - RETURN VALUE:
					//     . ERROR_SUCCESS						: 성공
					//     . 기타								: 기타 리턴값에 대해서는 핵쉴드 프로그래밍 가이드 참고바람.
					...
					TRACE("[%x] = _AhnHS_MakeAckMsg()", dwRet);

					// Header를 작성한다.
					header.opCode = OP_ACKCRC;
					header.length = htonl ( SIZEOF_ACKMSG );
		...
		{
			// [구 서버연동] byAckMsg 바이트 배열을 사용한다.
			pbyResponse = byAckMsg;
		}
